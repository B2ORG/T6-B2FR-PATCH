...
SetCharacters()
{
		...
		if ( enablesurvival )
		{
			...
			if ( level.players.size > 1 )
			{
				...
				if ( level.players.size > 2) 
				{
					...
					players[2] setmodel( "c_zom_player_cdc_fb" );
					players[2].voice = "american";
					players[2].skeleton = "base";
					players[2] setviewmodel( cdcviewmodel );
					players[2].characterindex = 1;
	...
	else	// Else simplifies if statement for Tranzit
	{
		...
		{
			// Set white player properties
			players[0] setmodel( "c_zom_player_handsome_fb" );
			players[0].voice = "american";
			players[0].skeleton = "base";
			players[0] setviewmodel( "c_zom_handsome_sleeveless_viewhands" );
			level.vox maps/mp/zombies/_zm_audio::zmbvoxinitspeaker( "player", "vox_plr_", self );
			players[0].favorite_wall_weapons_list[ self.favorite_wall_weapons_list.size ] = "blundergat_zm";
			players[0] set_player_is_female( 0 );
			players[0].character_name = "Billy";
			players[0].characterindex = 2;

			if ( level.players.size > 1 )
			{
				// Set blue player properties
				players[1] setmodel( "c_zom_player_oleary_fb" );
				players[1].voice = "american";
				players[1].skeleton = "base";
				players[1] setviewmodel( "c_zom_oleary_shortsleeve_viewhands" );
				level.vox maps/mp/zombies/_zm_audio::zmbvoxinitspeaker( "player", "vox_plr_", self );
				players[1].favorite_wall_weapons_list[ self.favorite_wall_weapons_list.size ] = "judge_zm";
				players[1] set_player_is_female( 0 );
				players[1].character_name = "Finn";
				players[1].characterindex = 0;

				if ( level.layers.size > 2) 
				{			
					// Set yellow player properties
					players[2] setmodel( "c_zom_player_arlington_fb" );
					players[2].voice = "american";
					players[2].skeleton = "base";
					players[2] setviewmodel( "c_zom_arlington_coat_viewhands" );
					level.vox maps/mp/zombies/_zm_audio::zmbvoxinitspeaker( "player", "vox_plr_", self );
					players[2].favorite_wall_weapons_list[ self.favorite_wall_weapons_list.size ] = "ray_gun_zm";
					players[2] set_player_is_female( 0 );
					players[2].character_name = "Arlington";
					players[2].characterindex = 3;

					if ( level.players.size > 3 )
					{
						// Set green player properties
						players[3] setmodel( "c_zom_player_deluca_fb" );
						players[3].voice = "american";
						players[3].skeleton = "base";
						players[3] setviewmodel( "c_zom_deluca_longsleeve_viewhands" );
						level.vox maps/mp/zombies/_zm_audio::zmbvoxinitspeaker( "player", "vox_plr_", self );
						players[3].favorite_wall_weapons_list[ self.favorite_wall_weapons_list.size ] = "thompson_zm";
						players[3] set_player_is_female( 0 );
						players[3].character_name = "Sal";
						players[3].characterindex = 1;
					}
				}
			}
