...
safe_init()
{
    ...
    level.FRFIX_PLUGIN_ZONES = ::zone_hud;
}
...
zone_hud(func_hud_properties, raw_names)
{
	...
	if (!isDefined(level.FRFIX_CONFIG["vanilla"]))
		return;
	else if (!level.FRFIX_CONFIG["vanilla"])
		return;

    if (!isDefined(raw_names))
        raw_names = false;
	...
	self.hud_zone [[func_hud_properties]]("hud_zone", "CENTER", "BOTTOM", 0, 20);
	self.hud_zone.alpha = 0.8;
	self.hud_zone.hidewheninmenu = 1;
    ...
    while (true)
    {
        ...
        self thread show_zone(new_zonename, self.hud_zone, raw_names);

        old_zonename = new_zonename;
    }
}

show_zone(zone, hud, rawnames)
{
    level endon("end_game");
    self endon("disconnect");
...
// Taken from https://github.com/Jbleezy/BO2-Reimagined
translate_zone(zone)
{
	if (!isDefined(zone))
	{
